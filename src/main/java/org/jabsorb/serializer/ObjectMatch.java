...
/**
 * <p>
 * This class is returned from the Serializer tryUnmarshall method to indicate number of mismatched
 * fields. This is used to handle ambiguities with JavaScript's typeless objects combined with and
 * Java's operator overloading.
 * </p>
 * TODO: wouldn't a better name for this class be ObjectMismatch as it's would be more descriptive.
 * The name ObjectMatch is a little confusing because it implies the opposite of what the class
 * actually stores (ObjectMismatch) either that, or I'm not understanding something correctly...
 * [WB: I agree!]
 */
public class ObjectMatch {
  ...
  /**
   * The objects can be converted into the same types, eg int converts into a string.
   */
  public static final ObjectMatch SIMILAR = new ObjectMatch(-2);

  /**
   * The objects can be converted into the same types, but it should be avoided eg any string can
   * converts into a boolean, where "true" is true and anything else is false.
   */
   ...
   */
  public static final ObjectMatch NO_MATCH = new ObjectMatch(Integer.MAX_VALUE);
  ...
  /**
   * Set the mismatch on this ObjectMatch. The ObjectMatch cannot be immutable anymore (at least in
   * the current design-- because the same mismatch object must be maintained through recursive
   * processing to properly handle circular references detection)
   *
