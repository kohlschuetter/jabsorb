...
/**
 * Used by Serializers to hold state during marshalling and unmarshalling. It keeps track of all
 * Objects encountered during processing for the purpose of detecting circular references and/or
 * duplicates.
 */
public class SerializerState {
  /**
   * The key is the processed object. The value is a ProcessedObject instance which contains both
   * the object that was processed, and other information about the object used for generating
   * fixups when marshalling.
   */
  ...
  /**
   * Represents the current json location that we are at during processing. Each time we go one
  ...
  /**
   * Add a fixup entry. Assumes that the SerializerState is in the correct scope for the fix up
   * location.
   *
   ...
   * @param ref reference to object within parent-- should be a String if parent is an object, and
   *          Integer if parent is an array. Can be null if this is the root object that is being
   *          pushed/processed.
   */
   ...
   * Currently only used within tryUnmarshall and unmarshall. This MUST be called before a given
  ...
  public void setSerialized(Object source, Object target) throws UnmarshallException {
    ...
    if (p == null) {
      ...
      throw new UnmarshallException("source object must be already registered as a ProcessedObject "
          + source);
    }
